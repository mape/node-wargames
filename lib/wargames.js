	...
	Object.keys(self.users).forEach(function (name) {
		...
		if (user.lat === undefined && user.ip) {
			(function (user) {
				...
				throw new Error('You will have to find a service that allows you to find the location of IPs. When you have done that, remove this line.');
				request({'uri':'http://some.geoip.service.com/?ip='+user.ip}, function(err, res, body) {
					body.split('<tr>').forEach(function(row) {
						if (row.match(/Latitude/)) {
							var tmpLat = row.match(/-?[0-9]+\.?([0-9]+)?/);
							if (tmpLat && tmpLat[0]) {
								lat = tmpLat[0];
							}
						}
						if (row.match(/Longitude/)) {
							var tmpLng = row.match(/-?[0-9]+\.?([0-9]+)?/);
							if (tmpLng && tmpLng[0]) {
								lng = tmpLng[0];
							}
						}

	...
	function ircMap(options) {
		...
		function createMarker(user) {
			if (user && !markers[user]) {
				...
				var $userListEntry = $('<li>' + id + '</li>').hover(function () {
		...
		function updateChannelActivity(time) {
			...
			if (!hasSetWidth) {
				$('#messages, #last-activity').width($('#last-activity').width() + 50);
				hasSetWidth = true;
			}
		}
		...
		function addMessages(messages) {
			...
			var $messageContainer = $('#messages').width(width);
			$.each(messages, function (index) {
				...
				$messageContainer.prepend($('<li><span class="user">' + user + ':</span><span class="message"></span></li>').find('.message').text(message).end());
			});
			$messageContainer.find('li:gt(2)').remove();
			$messageContainer.find('li').each(function (index) {
				$(this).css('opacity', ((0.6 / 3) * (3 - index)));
			});
		}
		...
		function initWebsocketConnection() {
			...
			server.on('message', function(msg) {
				...
				case 'getUsers':
					...
					$('title').text(data.channel + ' - IRC monitoring - WarGames!');
					users = data.users;
